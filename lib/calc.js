...
// 單筆紀錄的「滿意度比例」：滿意度相對於投入程度的比值。
// 投入 1、滿意度 1 → 比例 1（=100%，剛好符合預期的小投入小滿足）；
// 投入 5、滿意度 1 → 比例 0.2（投入很多卻回報很少）；
// 投入 1、滿意度 5 → 比例 5（超出預期的高回報）
function satisfactionRatio(e) {
  if (e.satisfaction === null || e.satisfaction === undefined) return null;
  const intensity = Number(e.intensity) || 1;
  return Number(e.satisfaction) / intensity;
}

// 計算某一段期間(periodStart ~ periodEnd，含首尾)內，六個向度各自的：
// investment(投入總量，來自 intensity，一件事可複選多個向度，各自都算全額) 跟
// satisfaction(已評分項目的滿意度總和/平均，來自 satisfaction，可能還沒評)
function computePeriodBreakdown(entries, periodStart, periodEnd) {
  ...
  const result = {};
  DIMENSIONS.forEach((d) => { result[d] = { investment: 0, satisfactionSum: 0, satisfactionCount: 0, ratioSum: 0, entryCount: 0 }; });

  for (const e of entries) {
    const dateStr = toDateStr(e.event_date);
    if (dateStr < startStr || dateStr > endStr) continue;
    const ratio = satisfactionRatio(e);
  ...
  DIMENSIONS.forEach((d) => {
    result[d].avgSatisfaction = result[d].satisfactionCount > 0
      ...
      : null;
    result[d].avgRatio = result[d].satisfactionCount > 0
      ? Math.round((result[d].ratioSum / result[d].satisfactionCount) * 100) / 100
      : null;
  });
...
// 全時間平均「滿意度比例」（給城市建築「色澤」用）：比例 <1 代表投入大於回報、
// =1 代表剛好符合預期、>1 代表超出預期。完全沒評過時給中性值 1（=符合預期）
function computeCumulativeSatisfaction(entries) {
  ...
  const avg = {};
  DIMENSIONS.forEach((d) => {
    avg[d] = counts[d] > 0 ? Math.round((sums[d] / counts[d]) * 100) / 100 : 1;
  });
  return avg;
}

// 🎛️ 投入很高、但滿意度比例偏低的提醒門檻（想調整就改這兩個數字）
const SATISFACTION_RATIO_LOW_THRESHOLD = 0.6; // 平均只拿回投入的 6 成以下，視為報酬偏低
const MIN_RATED_FOR_SATISFACTION_ALERT = 2;

// 這一期投入最多的向度，如果平均滿意度比例偏低，跳出提醒（跟失衡提醒是分開的兩種訊號）
function computeSatisfactionAlert(breakdown) {
  const withData = DIMENSIONS
    .map((d) => ({ dimension: d, ...breakdown[d] }))
    .filter((d) => d.satisfactionCount >= MIN_RATED_FOR_SATISFACTION_ALERT && d.investment > 0);
  if (withData.length === 0) return null;

  withData.sort((a, b) => b.investment - a.investment);
  const top = withData[0];
  if (top.avgRatio !== null && top.avgRatio < SATISFACTION_RATIO_LOW_THRESHOLD) {
    return { dimension: top.dimension, avgSatisfaction: top.avgSatisfaction, avgRatio: top.avgRatio };
  }
  return null;
}
