...
// 本週規劃：每一項只計入它「當天」的負擔，不做前後淡化/預期分布
// （這跟「事件」分頁的 acute/chronic 型是分開的兩套邏輯）
function computeWeeklyBurdenHistogram(items, weekStartDate) {
  ...
  dayKeys.forEach((key, idx) => {
    ...
    const dayStr = startOfDay(dayDate).toISOString().slice(0, 10);

    let burdenSum = 0;
    for (const item of items) {
      const itemDateStr = startOfDay(new Date(item.event_date)).toISOString().slice(0, 10);
      if (itemDateStr === dayStr) {
        burdenSum += Number(item.initial_burden) || 0;
      }
    }
