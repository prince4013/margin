...
// investment(投入總量，來自 intensity) 跟 satisfaction(已評分項目的滿意度總和/平均，來自 satisfaction，可能還沒評)
function computePeriodBreakdown(entries, periodStart, periodEnd) {
  ...
  for (const e of entries) {
    ...
    result[e.dimension].investment += Number(e.intensity) || 0;
    result[e.dimension].entryCount += 1;
    if (e.satisfaction !== null && e.satisfaction !== undefined) {
      result[e.dimension].satisfactionSum += Number(e.satisfaction);
      result[e.dimension].satisfactionCount += 1;
    }
  }
...
function computeCumulativeSatisfaction(entries) {
  ...
  for (const e of entries) {
    ...
    sums[e.dimension] += Number(e.satisfaction);
